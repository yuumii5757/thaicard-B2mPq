const App = {
    ...
    loadSettings() {
        const saved = localStorage.getItem('thaicard_settings');
        if (saved) {
    ...
    saveSettings() {
        localStorage.setItem('thaicard_settings', JSON.stringify(this.settings));
        this.applyTheme();
    },
    ...
    renderManage(params) {
        ...
        document.getElementById('btnExport').addEventListener('click', () => {
            ...
            downloadAnchorNode.setAttribute("download", "thaicard_backup.json");
        ...
        const modalHtml = `
            <div class="modal-overlay active" id="cardModal">
                <div class="modal-content">
                    ...
                    <div class="form-group mb-3">
                        <label class="block mb-1 text-sm font-bold">タイ語 (正解) <span class="text-danger">*</span></label>
                        <input type="text" id="mTarget" class="form-control thai-font text-lg" placeholder="例: สวัสดี" value="${c.target_text}">
                    </div>

                    <div class="form-group mb-3">
                        <label class="block mb-1 text-sm font-bold">発音記号</label>
                        <input type="text" id="mPronunciation" class="form-control" placeholder="例: sawàtdii" value="${c.pronunciation || ''}">
                    </div>
    ...
    renderResult() {
        ...
        if (cq.wrongCards.length > 0) {
            ...
            cq.wrongCards.forEach(c => {
                ...
                el.innerHTML = `
                    <div class="list-item-title">${c.native_text}</div>
                    <div class="list-item-sub thai-font text-primary">${c.target_text} <span class="text-muted ml-2">${c.pronunciation || ''}</span></div>
                `;
    ...
    renderSettings() {
        ...
        const populateVoices = () => {
            ...
            // Filter primarily for Thai, but keep others as fallback
            const thaiVoices = voices.filter(v => v.lang.startsWith('th'));
            const showVoices = thaiVoices.length > 0 ? thaiVoices : voices;
        ...
        document.getElementById('btnTestVoice').addEventListener('click', () => {
            this.speakText('สวัสดีครับ ยินดีที่ได้รู้จัก');
        });
